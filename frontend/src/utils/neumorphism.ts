...
function adjustBrightness(color: RGB, percent: number): string {
  const r = Math.max(
    0,
    Math.min(255, Math.floor(color.r * (1 + percent / 100))),
  );
  const g = Math.max(
    0,
    Math.min(255, Math.floor(color.g * (1 + percent / 100))),
  );
  const b = Math.max(
    0,
    Math.min(255, Math.floor(color.b * (1 + percent / 100))),
  );
  return `rgb(${r},${g},${b})`;
}

function generateNeumo(hexColor: string): {
  borderRadius: string;
  background: string;
  boxShadow: string;
} {
